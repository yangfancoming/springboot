...
public class SocketDecoder extends CumulativeProtocolDecoder {
    ...
    protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws IOException {
        ...
        log.info("接收数据总长度:= {} 接收数据内容: ={}",in.remaining() , ByteArrayUtils.byteArrToHexString(data));
        ...
        length[0] = data[2];
        length[1] = data[3];
        short i = ByteArrayUtils.byteArrToShort(length);
        if (i != data.length) {
        ...
        log.info("起始/结束 标识处理器:解析结果（16进制）：" + ByteArrayUtils.byteArrToHexString(start));
        ...
        log.info("包类型处理器:  解析结果（16进制）：" + ByteArrayUtils.byteArrToHexString(type));
        out.write(input);
        return true;
    }
}
